...
let start_counter = 1
let next_counter = 1
let hookRunner_counter = 1
let hookRunner_application_counter = 1
let avvio_error_map_count = 1
let avvio_error_ready_timeout_count = 1
...
function hookRunnerApplication (hookName, boot, server, cb) {
  console.log('\nHookRunner in hooks.js #', hookRunner_counter++ )
  ...
  next()
   console.log('\nafter next at start of hookrunner - hooks.js #', next_counter++, ' line 108')
  ...
  function exit (err) {
    if (err) {
      ...
      if (err.code === 'AVV_ERR_READY_TIMEOUT') {
        ...
        console.log('avvio_error_timeout_count - hooks.js line 119: ', avvio_error_ready_timeout_count++)
      } else {
        err = AVVIO_ERRORS_MAP[err.code] != null
          ? appendStackTrace(err, new AVVIO_ERRORS_MAP[err.code](err.message))
          : err

          console.log('avvio_error_map_count - hooks.js line 125 ', avvio_error_map_count++)
      }
  ...
  function next (err) {
    ...
    console.log('\nstart of function next - hooks.js #', start_counter++, ' in line 127')
    if (err) {
      exit(err)
      return
    }
